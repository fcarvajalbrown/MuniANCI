...
- `POST /chat` — the core endpoint. Builds a **topic-aware retrieval query** from the recent user turns (via `_retrieval_query`, so multi-turn follow-ups like "menciona 5 ejemplos" keep the conversation topic instead of retrieving on the bare phrase), calls `rag.retrieve()`, injects the retrieved legal text into an augmented user message, and sends the model's response back as SSE. The stream sends a `citations` event first (so the frontend can render sources immediately), then the answer, then a `done` event. **The answer is not streamed token by token to the client**: the model's tokens are accumulated, `citas.py` runs over the complete text, and only then does a single `token` event carry the answer or the decline. A guard that cleared a fabricated citation after the user had already read it would not be a guard.
...
**`rag.py`** — hybrid retrieval, the heart of the system. `buscar_en(tabla, consulta, embedding, limite)` is the single retrieval path: it runs **both** vector search and BM-25 full-text search (`CANDIDATOS=20` per side) against a given LanceDB table and fuses the two rankings with **RRF** (`RRF_K=60`) down to `limite`. `retrieve()` embeds the query via `inference.py` and calls it with `TOP_K=5` against the table `db_dir()` resolves; the orchestrator calls the same function per corpus and then `fusionar()`. There is deliberately no second implementation to keep in step — `buscar_en` reached parity with `retrieve()` on 2026-08-12 by absorbing it, not by copying it. Since 0.9.0 item 5 there is a deterministic step in front of the fusion: `ruta_articulo()` reuses `citas.articulos()` to detect an article number in the query and injects up to `ARTICULO_SLOTS=3` chunks of that article, **restricted to the files the hybrid search already returned** and ordered by their source's rank, so it can never answer with the homonymous article of a law retrieval never considered (17 of 22 corpus files have an artículo 9). It abstains when the query names more than two articles, when there are no candidates, or when the DB predates schema v2. FTS degrades gracefully to empty if the index is missing. LanceDB is synchronous, so the searches run sequentially. Not a standalone script — imported by main.py.

**`citas.py`** — the last line of defence, deterministic and independent of the model. Every article number **and every URL** in the generated answer must appear in a retrieved fragment; if any does not, the whole answer is replaced by a fixed decline naming what could not be backed. The URL half was added on 2026-08-12 after a fabricated `ayuntamiento.providencia.cl` link passed the guard untouched: a false URL is an invented reference as much as a false article, with the aggravation that it looks verifiable. The 24 corpus files contain no URLs at all, so in practice the rule blocks any answer carrying a link. Known and deliberate limit on the article half: numbers are compared without the norma they belong to, so an artículo 9 of another law present in the context backs a citation to artículo 9.
...
questions recall@k=0.6667, MRR=0.5, mean_precision=0.1333, nDCG@k=0.5436. Same day, the Tramo A
progression, all on `db/`:
